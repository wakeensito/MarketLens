...
def lambda_handler(event: dict, context) -> dict:
    expected = (
        event.get("request", {}).get("privateChallengeParameters", {}).get("code")
    )
    ...
    logger.info(
        ...
        extra={
            "has_private_params": bool(
                event.get("request", {}).get("privateChallengeParameters")
            ),
    ...
    if not expected or not isinstance(expected, str):
        ...
        logger.warning(
            "OTP verification missing or invalid expected code in privateChallengeParameters"
        )
        return event
