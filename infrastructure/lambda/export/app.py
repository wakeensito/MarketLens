...
def generate_markdown(report: dict) -> str:
    """Generate a Markdown string from report data."""
    result = report.get("result_json", {})
    idea = report.get("idea_text", "")
    lines = []

    lines.append(f"# Market Analysis: {idea}")
    lines.append("")
    lines.append(f"**Generated:** {report.get('created_at', '')}")
    lines.append(f"**Vertical:** {result.get('vertical', 'N/A')}")
    lines.append(f"**Business Model:** {result.get('business_model', 'N/A')}")
    lines.append("")
    ...
    competitors = result.get("competitors", [])
    if competitors:
        lines.append(f"## Competitors ({len(competitors)} found)")
        lines.append("")
    ...
    if roadmap:
        lines.append("## Entry Roadmap")
        lines.append("")
        for r in roadmap:
            lines.append(f"### {r.get('phase', '')} — {r.get('title', '')}")
            lines.append(f"{r.get('description', '')}")
            lines.append("")

    # Trend + Recommendation
    trend = result.get("trend_signal", "")
    if trend:
        lines.append("## Trend Signal")
    ...
    lines.append("---")
    lines.append("*Generated by Plinths — AI Market Intelligence*")
...
def export_report(report_id: str):
    ...
    # Generate export
    if export_format == "md":
        # Markdown is free for all plans
        content = generate_markdown(item)
