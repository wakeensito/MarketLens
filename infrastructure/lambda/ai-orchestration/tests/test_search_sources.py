...
def test_top_sources_labels_by_domain_and_dedupes():
    results = [
        ...
        {
            "title": "Statista fitness",
            "url": "https://www.statista.com/x",
            "description": "",
        },
        {
            "title": "dupe",
...
def test_top_sources_no_subdomain_false_match():
    ...
    out = _top_sources(
        [{"title": "x", "url": "https://notreddit.com/a", "description": ""}], n=1
    )
    assert out[0]["label"] != "Reddit"
    assert out[0]["label"] == "Notreddit"
    # a real reddit subdomain still maps
    out2 = _top_sources(
        [{"title": "x", "url": "https://old.reddit.com/r/x", "description": ""}], n=1
    )
