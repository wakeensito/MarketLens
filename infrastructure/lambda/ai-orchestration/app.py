...
def score(parsed: dict, analysis: dict, search_results: dict) -> dict:
    ...
    # Continuous competitor scaling: 0 competitors → 0, 20+ → 40 (smooth curve)
    competitor_count_factor = min(40.0, num_direct * (40.0 / 16.0))  # linear up to 16, capped at 40
    ...
    if funding_concentrated:
        ...
        funding_factor = 3.0
    # Boost if both public AND concentrated
    ...
    if growth_pct is not None and growth_pct > 0:
        # 5% → 5, 15% → 15, 25% → 22, 40%+ → 28 (diminishing returns)
