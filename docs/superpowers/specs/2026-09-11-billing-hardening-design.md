...
| **Stripe API call fails during refetch** (5xx, rate limit, network) | Raise → 500 → Stripe retries. The Stripe client gets a 5 s request timeout and `max_network_retries = 0`. The worst case is four sequential calls (refetch the newcomer, the `Customer.retrieve` identity fallback, refetch the recorded subscription, the cancel): 4 × 5 s = **20 s**, under API Gateway's 29 s REST integration cap and the 30 s Lambda timeout. One SDK retry per call would make the same path ≈ 42 s and blow both caps; Stripe's own webhook redelivery is the outer retry that matters |
