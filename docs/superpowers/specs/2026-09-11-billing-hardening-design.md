...
| **Stripe API call fails during refetch** (5xx, rate limit, network) | Raise → 500 → Stripe retries. The Stripe client gets a 5 s request timeout and `max_network_retries = 0`. The worst case is five sequential calls (refetch the newcomer, the `Customer.retrieve` identity fallback, refetch the recorded subscription, the cancel, and — on a failed cancel — the confirmation refetch): 5 × 5 s = **25 s**, under API Gateway's 29 s REST integration cap and the 30 s Lambda timeout. SDK retries are off entirely; Stripe's own webhook redelivery is the outer retry that matters |
