...
- Each stage has its own API key and webhook secret (SSM SecureString). Price IDs are currently one set in `template.yaml`; test-mode and live-mode prices are different Stripe objects, so before the first prod deploy with `STRIPE_LIVEMODE=true` verify the four IDs are live-mode objects (or `Session.create` fails).
