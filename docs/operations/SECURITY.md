...
- Each stage has its own API key and webhook secret (SSM SecureString). **`STRIPE_LIVEMODE` and the four price IDs are template parameters** (`StripeLivemode`, `StripePriceIdPro`, `StripePriceIdMax`, `StripePriceIdProAnnual`, `StripePriceIdMaxAnnual`), not stage-derived values. They must all match the mode of the Stripe secret key in SSM for that stage: a live-mode key with `StripeLivemode=false` rejects every real webhook, and a test-mode price id against a live-mode key makes `Session.create` fail. Today `dev` runs a live-mode key, so `samconfig.toml` pins `StripeLivemode="true"` explicitly.
- **Test-mode QA pass:** put a test-mode secret key + webhook secret in the stage's SSM parameters, then deploy with `sam deploy --parameter-overrides Stage="dev" CognitoCallbackDomain="plinths.net" StripeLivemode=false StripePriceIdPro=<test price> StripePriceIdMax=<test price> StripePriceIdProAnnual=<test price> StripePriceIdMaxAnnual=<test price>`. Restore the live-mode key and redeploy with the defaults when the pass is done — `bin/deploy` (which reads the stage's `stripe-secret-key` from SSM and derives `StripeLivemode` from its `sk_live_` / `sk_test_` prefix, refusing to deploy if an inherited `STRIPE_LIVEMODE` disagrees) or a bare `sam deploy` (which uses `samconfig.toml`'s pinned `StripeLivemode="true"`) — leaving the stack in test mode silently drops real webhooks.
- The dashboard endpoint must subscribe to exactly these seven event types: `checkout.session.completed`, `customer.subscription.created`, `customer.subscription.updated`, `customer.subscription.paused`, `customer.subscription.resumed`, `customer.subscription.deleted`, `invoice.payment_failed`. `.created` / `.paused` / `.resumed` are new in this design — an endpoint still configured for the old four leaves paused subscriptions with paid access.
- Alarms worth wiring: `WebhookLivemodeMismatch`, `UnknownPriceId`, `DoubleSubscriptionCancelled` (manual refund runbook), `DoubleSubscriptionCancelFailed` (a duplicate subscription may still be billing and Stripe is redelivering), `WebhookOrphanUser`, `WebhookStaleEvent`. `WebhookUnknownSubscription` is emitted alongside these but is informational, not alarm-worthy — Stripe routinely delivers `.updated` / `invoice.*` for a subscription before the install lands, so it fires during normal signup ordering. `WebhookStaleEvent` is the one that means an event was genuinely dropped.
